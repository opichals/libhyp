...
function insertImages(lines) {
  ...
  for (let args of limg_args) {
    if (args.xoffset == 0) {
      ...
      $images += `<div class="img" style="left:${Math.round(
        (hyp_width / MAX_WIDTH) * (args.xoffset - 1)
      )}; z-index:${z};"><div><pre>`;
    }
    ...
    if (args.type === 'limage') {
      ...
      lines.splice($begidx + args.ytextoffset, 0, $limgnl);
      $begidx += 1;
    }
...
function htmllify(file, lines, lineNumber) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    lines[i] = $l.replace(
      /<!--(\S+)\s+\"([^\"]+)\"-->\n/g,
      (m, cmd, args) => (graphics.push(`cmd=${cmd}&${args}`), '')
    );
    $begidx++;
  }
  ...
  $Lines = $Lines.replace(
    /(\s)([a-z]+[a-z0-9.\-_]+\@[a-z0-9.\-_]*[a-z])([;:,\.\]\)\}\"\'<>]*\s)/gim,
